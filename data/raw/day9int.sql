...
#Refine and optimize queries for dashboard performance

select * from activitylogs; 
#(returns all  50,000 rows unnecessarily and slows down the query performance)

select count(distinct userid) as mau
from activitylogs; 
#(returns only required monthly active users)

#Create temporary tables for Daily Active User (DAU) trends
